...
Begin VB.Form Form1 
   ...
   Begin VB.Menu mnuExtra 
      ...
      End
   End
   Begin VB.Menu Ambientaciones 
      Caption         =   "Ambientaciones"
      Begin VB.Menu ver 
         Caption         =   "Ver"
      End
      Begin VB.Menu GuardarAmb 
         Caption         =   "Guardar"
      End
      Begin VB.Menu RecargarAmb 
         Caption         =   "Recargar"
      End
   End
...
'If Frames > 0 And IsNumeric(Frames) Then
    Do
    DoEvents
    If Ocupado = True Then Exit Do
    If IsNumeric(Frames) = False Then Exit Sub
...
sx.Locked = False
sy.Locked = False
pixelWidth.Locked = False
pixelHeight.Locked = False

sx.BackColor = vbRed
sy.BackColor = vbRed
pixelWidth.BackColor = vbRed
pixelHeight.BackColor = vbRed

Else
editando = False
sx.Locked = True
sy.Locked = True
pixelWidth.Locked = True
pixelHeight.Locked = True
sx.BackColor = &H4000&
sy.BackColor = &H4000&
pixelWidth.BackColor = &H4000&
pixelHeight.BackColor = &H4000&
End If
End Sub
...
DoEvents
Call CargarCascos
Carga.Label1.Caption = "Cargando Ambientaciones..."
DoEvents
...
Cred.Caption = "Programado por ^[GS]^ - Website: http://www.gs-zone.com.ar - Soporte 0.12.1 por Ladder"
End Sub
...
Private Sub GuardarAmb_Click()
Call GrabarAmbientacion
End Sub

Private Sub killgrh_Click()
Dim i As Long
...
If GrhData(i).FileNum > 0 Then
    ...
    If LenB(Dir(DirClien & "\graficos\" & GrhData(i).FileNum & ".png", vbArchive)) = 0 Then
        Debug.Print "No existe el " & GrhData(i).FileNum & ".png"
        GrhData(i).NumFrames = 0
        
        Datos = Datos & "Falta grafico: " & GrhData(i).FileNum & ".png Del GRH: " & i & vbCrLf
        cuantos = cuantos + 1
    End If
End If
...
If LenB(Dir(DirClien & "\Graficos\" & FileName & ".png", vbArchive)) = 0 And Frames <= 1 Then
    GRHt.Text = "ERROR: Falta la imagen " & FileName & ".png en Graficos."
    Exit Sub
End If
Call ConvertFileImage(DirClien & "\Graficos\" & FileName & ".png", App.Path & "\temp\" & FileName & ".jpg", 100)
Ix.Picture = LoadPicture(App.Path & "\temp\" & FileName & ".jpg")
...
pixelWidth = GrhData(Index).pixelWidth
pixelHeight = GrhData(Index).pixelHeight
If LenB(Dir(DirClien & "\Graficos\" & FileNameX & ".png", vbArchive)) = 0 Then
    If GRHt.Text <> "ERROR: Falta la imagen " & FileNameX & ".png en Graficos." Then
        GRHt.Text = "ERROR: Falta la imagen " & FileNameX & ".png en Graficos."
    End If
    Exit Function
End If


Call ConvertFileImage(DirClien & "\Graficos\" & FileNameX & ".png", App.Path & "\temp\" & FileNameX & ".jpg", 100)



Ix.Picture = LoadPicture(App.Path & "\temp\" & FileNameX & ".jpg")
...
If Button = vbRightButton Then
    PopupMenu frmAmbientaciones.menuamb
End If
End Sub
...
For i = 1 To NumFxs
    If FxData(i).FX.grhindex > 0 Then
        ...
        Datos = Datos & "Animacion=" & FxData(i).FX.grhindex & vbCrLf & "OffsetX=" & FxData(i).OffsetX & vbCrLf & "OffsetY=" & FxData(i).OffsetY & vbCrLf & "IsPNG=" & FxData(i).IsPng & vbCrLf & vbCrLf
    End If
Next
...
If LenB(Dir(DirExpor & "\Graficos.ini", vbArchive)) = 1 Then
Call Kill(DirExpor & "\Graficos.ini")
End If

n = FreeFile
Open DirExpor & "\Graficos.ini" For Binary Access Write As n
Put n, , "[INIT]" & vbCrLf & "NumGrh=" & MaxGRH & vbCrLf & vbCrLf
K = 0
...
For i = 1 To MaxGRH
    If GrhData(i).NumFrames > 1 Then
        ...
        For j = 1 To GrhData(i).NumFrames
            ...
            ElseIf LenB(Dir(DirClien & "\Graficos\" & GrhData(GrhData(i).Frames(j)).FileNum & ".png", vbArchive)) = 0 Then
...
Dim EjFx() As tIndiceFx
If UsarGrhLong = False Then
    Dim EjFxInt(1 To 254) As tIndiceFxInt
End If


For i = 1 To K
    If UsarGrhLong = True Then
        EjFx(i).OffsetY = LeerINI.DarValor("FX" & i, "OffsetY")
        EjFx(i).OffsetX = LeerINI.DarValor("FX" & i, "OffsetX")
        EjFx(i).Animacion = LeerINI.DarValor("FX" & i, "Animacion")
        EjFx(i).IsPng = LeerINI.DarValor("FX" & i, "IsPng")
        Put #n, , EjFx(i)
       
        
    Else
        
        EjFxInt(i).OffsetY = LeerINI.DarValor("FX" & i, "OffsetY")
        EjFxInt(i).OffsetX = LeerINI.DarValor("FX" & i, "OffsetX")
        EjFxInt(i).Animacion = LeerINI.DarValor("FX" & i, "Animacion")
        EjFxInt(i).IsPng = LeerINI.DarValor("FX" & i, "IsPng")
        Put #n, , EjFxInt(i)
    End If
...
Private Sub pixelHeight_Change()
If editando Then
If IsNumeric(pixelHeight) Then
GrhData(Index).pixelHeight = pixelHeight
Call Listado_Click
End If
End If
End Sub

Private Sub pixelWidth_Change()
If editando Then
If IsNumeric(pixelWidth) Then
GrhData(Index).pixelWidth = pixelWidth
Call Listado_Click
End If
End If
End Sub

Private Sub RecargarAmb_Click()
Call LeerAmbientaciones
End Sub
...
If editando Then
If IsNumeric(sx.Text) Then
 GrhData(Index).sx = sx.Text
    Call Listado_Click
End If
End If
End Sub
...
If editando Then
If IsNumeric(sy.Text) Then
GrhData(Index).sy = sy.Text
Call Listado_Click
End If
End If
End Sub
